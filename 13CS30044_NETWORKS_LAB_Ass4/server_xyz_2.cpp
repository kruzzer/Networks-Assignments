...
int check_name(char username[] )
{
  ...
  if( mysql_query(con, "SELECT * FROM xyz") ) 
  {
...
int get_emails(char username[])
{
	...
	if( mysql_query(con, "SELECT * FROM xyz_mail ORDER BY id DESC") ) 
	{
	  cout << "mysql_query error" << endl ;
	  exit(1) ;
	}
...
void my_sql_same( char from_email[] , char to_email[] , char message[] ) 
{
	...
	char QUERRY[4000] ;
    strcpy(QUERRY , "INSERT INTO xyz_mail(from_email , to_email , message) VALUES('") ;
...
int main(int argc , char * argv[])
{
	if(argc < 4)
	{
		cout << "you need to give arguments <IP_ADDRESS of abc> <port for client> <port for abc>" << endl ;
		exit(1) ;
	}
    ...
    while(1)
    {
       ...
       cout << "Connection from an xyz client accepted " << endl ;
            ...
            if(choice == 1)
            {
            	while(1)
            	{
                   ...
                   // cout << to_email + strlen(to_email)-8 << endl ;
                    ...
                    if( !strcmp(&to_email[strlen(to_email)-8] , "@xyz.com") )
                    {	
